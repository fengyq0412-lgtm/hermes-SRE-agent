...
再次执行 `hermes-web` 时，如果端口由同一用户、同一启动脚本的旧 Hermes 占用，程序会停止旧实例并在端口释放后自动启动新实例。旧实例中未完成的审查会中断，内存中的对话与 token 计数会清空。如果占用者是其他程序或无法确认身份，Hermes 不会终止它；请自行检查端口，或修改 `.env` 中的 `HERMES_WEB_PORT`。
...
源码审查单次最多调用 10 次工具，之后预留两次模型调用生成最终报告；最多列出 500 个文件，每次最多读取 200 行，超出范围会提示截断。报告要求给出总体风险等级、已确认问题、待验证风险和审查范围。没有确凿缺陷时仍会说明残余风险，但不会把猜测写成已确认 Bug。若模型两次都不输出结论，页面仍会展示已读取范围和“待评估”的保底报告。回答基于实际读取范围，不代表已覆盖整个项目，也没有运行测试。页面显示简化后的检查进度，最终回答在模型返回后展示，暂非逐 token 输出。
